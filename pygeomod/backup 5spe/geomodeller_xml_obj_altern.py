    ...
    def change_formation_values_PyMC(self, **args):
        ...
        if args.has_key("contact_points_mc"):
            for contac_point_mc in args["contact_points_mc"]:
                try:
                    self.change_formation_point_pos(contact_points_dict[contac_point_mc[0]], y_coord = contac_point_mc[1])
                except KeyError:
        ...
        if args.has_key("azimuths_mc"):
            for azimuth_mc in args["azimuths_mc"]:
                ...
                try:
                    self.change_foliation(foliation_dict[azimuth_mc[0]], azimuth = str(azimuth_mc[1]))
                except KeyError:
        ...
        if args.has_key("dips_mc"):
            for dip_mc in args["dips_mc"]:
                try:
                    self.change_foliation(foliation_dict[dip_mc[0]], dip = str(dip_mc[1]))
                except KeyError:
